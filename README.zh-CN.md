...
- 把经验采集进复核队列，支持可选的本地蒸馏后端，并可幂等导入 Codex 已提炼的记忆；
- 通过 Codex 钩子或 DeepSeek Harness 原生适配器，在任务前检索记忆、在轮次结束采集最终回复；
- 通过本地 STDIO MCP Server 或 Streamable HTTP，向 Codex、Claude、Cursor、Gemini CLI、VS Code 等 Agent 开放知识库；
- 使用 `relic sync` 提交、拉取和推送知识库；
- 在浏览器中查看记忆、编辑条目与配置，并检查知识库健康状态。

0.2 至 1.1 的里程碑均已实现，详见[路线图](#路线图)。存储格式对后续里程碑保持兼容。
...
- **0.2（已完成）：** update/supersede CLI 命令、置信度衰减计算和更丰富的筛选条件
- **0.3（已完成）：** Streamable HTTP MCP 传输（JSON + SSE 流式、批量请求）、可选 Bearer 身份认证、`mcp-session-id` 会话和 OAuth 2.1 发现元数据
- **0.4（已完成）：** `relic sync` 的 Git 提交/拉取/推送，以及保留本地版本的确定性冲突解决
- **0.5（已完成）：** 反思自动触发、矛盾检测和模式提取
- **1.0（已完成）：** 配置 Schema 校验（`relic doctor`）、`relic watch` 维护守护进程，以及面向多设备工作流的配置化同步远端
- **1.1（已完成）：** 知识图谱——确定性本地矢量层、带证据的类型化关系、混合检索、MCP 图遍历与仪表盘关系视图
- **1.2（已完成）：** 持久化经验队列——采集与复核队列、Codex 与 DSH 生命周期钩子、带可选自动同步的独立守护进程、本地模型命令蒸馏，以及 Codex 已提炼记忆的导入
